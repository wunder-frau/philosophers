 ...
 *
 * This function updates the `init_time` field in the `t_table` structure.
 * If the number of successfully created philosopher threads (`i`)
 * matches the total number of philosophers (`table->size`),
 * it sets `init_time` to the current time. Otherwise, it sets `init_time` 
